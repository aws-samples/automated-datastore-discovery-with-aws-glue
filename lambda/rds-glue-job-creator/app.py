...
DDB_PARTIQL = "SELECT * FROM {} WHERE glue_job_created = False AND data_catalog_entry = True AND data_source_type = 'rds'"
...
GLUE_SCRIPT_FILE_NAME = "rds-source-script.py"
...
def lambda_handler(event, context):
    ...
    for obj in pythonic_results:
        ...
        connection_name = obj["data_source_attrs"]["connectionName"]
        LOGGER.info(f"Glue Data Catalog Connection Name: {connection_name}")
        
        # TODO: make region determination more robust
        region = os.environ["AWS_REGION"]

        glue_job_name = f"rds-pii-detect-{region}-{data_catalog_table_name}"
        create_glue_job(
            client=glue_client,
        ...
        wf_name = f"rds-wf-{region}-{data_catalog_table_name}"
        ...
        trigger_resp = glue_client.create_trigger(
            Name=f"rds-glue-trigger-{region}-{data_catalog_table_name}",
