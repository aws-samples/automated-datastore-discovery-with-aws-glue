...
DDB_PARTIQL = "SELECT * FROM {} WHERE glue_job_created = False AND data_catalog_entry = True AND data_source_type = 'dynamodb'"
...
GLUE_SCRIPT_FILE_NAME = "ddb-source-script.py"
...
def lambda_handler(event, context):
    ...
    for obj in pythonic_results:
        ...
        data_catalog_db_name = obj["data_catalog_db_name"]
        LOGGER.info(f"Glue Data Catalog Database Name: {data_catalog_db_name}")

        table_arn = obj["data_source_attrs"]["tableDescription"]["tableArn"]

        region = table_arn.replace("arn:aws:dynamodb:", "").split(":")[0]

        glue_job_name = f"dynamodb-pii-detect-{region}-{data_catalog_table_name}"
        ...
        create_glue_job(
            ...
            ddb_source_table_name=obj["data_source_attrs"]["tableDescription"]["tableName"],
            dc_table_name=data_catalog_table_name,
            output_table=output_table,
            dc_db_name=data_catalog_db_name,
            ddb_source_table_id=obj["data_source_attrs"]["tableDescription"]["tableId"],
            region=region,
            table_arn=table_arn,
            glue_job_name=glue_job_name
        )

        wf_name = f"dynamodb-wf-{region}-{data_catalog_table_name}"
        ...
        trigger_resp = glue_client.create_trigger(
            Name=f"dynamodb-glue-trigger-{region}-{data_catalog_table_name}",
